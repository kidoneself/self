package com.yimao.cloud.framework.interceptor;
...
public class FeignInterceptor implements RequestInterceptor {
    ...
    static {
        ignores.add("/server/serverPubKey");
        ignores.add("/server/userPubKey");
        ignores.add("/server/token");
        ignores.add("/server/allowed");
        ignores.add("/user/login");
        ignores.add("/admin/login");
        ignores.add("/dist/login");
        ignores.add("/distributor/login");
        ignores.add("/station/login");
    }
    ...
    public void apply(RequestTemplate requestTemplate) {
        ...
        if (!ignores.contains(url)) {
            try {
                // Feign请求传递带上权限认证信息
                requestTemplate.header(AuthConstants.JWTINFO, FeignRequestInterceptor.requestVariable.get());
