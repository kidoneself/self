...
public final class ContractUtil {
	...
	public Map<String, Object> signContract(String userName, String productModal, String snCode, String province,
			String city, String region, String address, boolean isNeedComplete, String completeNotice, int chargeType,
			int homeCostPrice, int homeOpenAccount, int busineseCostPrice, int busineseOpenAccount,
			int completeOpenAccount, String mobile, String identityCard, int yearAmount, String orderId, String email) {
		...
		ru = registerCreate(userName, productModal, snCode, province, city, region, address, isNeedComplete,
				completeNotice, chargeType, homeCostPrice, homeOpenAccount, busineseCostPrice, busineseOpenAccount,
				completeOpenAccount, mobile, identityCard, yearAmount, orderId, email);
		return ru;
	}
	...
	private YunSignResult checkidentityNumber(String identityNumber, String userName) {
		...
		try {
			return YunSignCaller.callYunSign(
					domainProperties.getYunSign() + "/identityCheck/webservice/IdentityCard?wsdl", "verifyIdentity",
					strs, val);
	...
	private Map<String, Object> registerCreate(String userName, String productModal, String snCode, String province,
			String city, String region, String address, boolean isNeedComplete, String completeNotice, int chargeType,
			int homeCostPrice, int homeOpenAccount, int busineseCostPrice, int busineseOpenAccount,
			int completeOpenAccount, String mobile, String identityCard, int yearAmount, String orderId, String email) {
		Map<String, Object> ru = new HashMap();
		...
		Map<String, Object> createResult = create(userName, productModal, snCode, province, city, region, address,
				isNeedComplete, completeNotice, chargeType, homeCostPrice, homeOpenAccount, busineseCostPrice,
				busineseOpenAccount, completeOpenAccount, mobile, identityCard, yearAmount, orderId, mobile);
	...
	private YunSignResult checkUserExisted(String userId) {
		...
		try {
			return YunSignCaller.callYunSign(domainProperties.getYunSign() + "/mmecserver3.0/webservice/Common?wsdl",
					"userQuery", strs, val);
	...
	private YunSignResult registerPersonalUser(String identityNumber, String userName, String mobile, String email,
			String userId, boolean ifAdmin) {
		String signType = "MD5";
		String appId = yunSignProperties.getWorkorderServiceAppid();
		String appSecretKey = yunSignProperties.getWorkorderServiceKey();
		String time = System.currentTimeMillis() + "";
		...
		try {
			YunSignResult result = YunSignCaller.callYunSign(
					domainProperties.getYunSign() + "/mmecserver3.0/webservice/Common?wsdl", "register", strs, val);
			return result;
	...
	public Map<String, Object> create(String userName, String productModal, String snCode, String province, String city,
			String region, String address, boolean isNeedComplete, String completeNotice, int chargeType,
			int homeCostPrice, int homeOpenAccount, int busineseCostPrice, int busineseOpenAccount,
			int completeOpenAccount, String mobile, String identityCard, int yearAmount, String orderId,
			String userId) {
		log.debug(
				"userName={1}; productModal={2}; snCode={3}; province={4};  city={5};  region={6};  address={7};  isNeedComplete={8}; completeNotice={9}; chargeType={10};\n         homeCostPrice={11}; homeOpenAccount={12}; busineseCostPrice={13}; busineseOpenAccount={14}; completeOpenAccount={15};\n         mobile={16};  identityCard={17}; yearAmount={18};  orderId={19};  userId{20}",
				new Object[] { userName, productModal, snCode, province, city, region, address, isNeedComplete,
						completeNotice, chargeType, homeCostPrice, homeOpenAccount, busineseCostPrice,
						busineseOpenAccount, completeOpenAccount, mobile, identityCard, yearAmount, orderId, userId });
		...
		try {
			...
			String data = YunSignData.contractData(userName, brand, productModal, snCode, province, city, region,
					address, costType, openAccount, completeNotice, homePrice, businessPrice, costChoose, costMoney,
					firstServiceCycle, homeMoney, businessMoney, cycle, accountBank, accountName, account,
					peroidOfValidity, year, month, day, validityYear, validityMonth, validityDay, stopPaymentDays,
					homeCompensate, firstParty, userName, representative, identityCard, firstPartyAddress,
					secondPartyAddress, firstPartyPhone, mobile, indemnity1, indemnity2, indemnity3, indemnity4,
					indemnity5, indemnity6, indemnity7, indemnity8);
			log.info("data:" + data);
			String customsId = userId + "," + YunSignConfig.YUNSIGN_SERVICE_YIMAO_ACCOUNT;
			YunSignResult createResult = createContract(userId, orderId, customsId, userName + "翼猫净水机使用合同",
					yunSignProperties.getServiceTempleid(), data);
			if (createResult.isSuccess()) {
				addUserSignInfo(new String[] { userId, YunSignConfig.YUNSIGN_SERVICE_YIMAO_ACCOUNT }, orderId,
						new String[] { "2", "1" });
	...
	public YunSignResult addUserSignInfo(String[] userId, String orderId, String[] postion) {
		...
		for (int i = 0; i < userId.length; ++i) {
			...
			signInfo = signInfo + "{\"userId\":\"" + userId[i] + "\",\"position\":\"" + postion[i]
					+ "\",\"signUiType\":\"1\" } ";
		}
		...
		try {
			return YunSignCaller.callYunSign(domainProperties.getYunSign() + "/mmecserver3.0/webservice/Addition?wsdl",
					"addSignInfo", strs, val);
		} catch (Exception var14) {
			return YunSignResult.err(var14.getMessage());
		}
	}

	private YunSignResult createContract(String userId, String orderId, String customsId, String title,
			String templateId, String data) {
		String signType = "MD5";
		String appId = yunSignProperties.getWorkorderServiceAppid();
		String appSecretKey = yunSignProperties.getWorkorderServiceKey();
		String time = System.currentTimeMillis() + "";
		...
		String[] strs = new String[] { "appId", "time", "sign", "signType", "userId", "customsId", "templateId",
				"orderId", "title", "offerTime", "data" };
		String[] val = new String[] { appId, time, sign, signType, userId, customsId, templateId, orderId, title,
				offerTime, data };
		try {
			YunSignResult result = YunSignCaller.callYunSign(domainProperties.getYunSign() + YunSignConfig.URI,
					"createContract", strs, val);
			if (result.isSuccess()) {
				return result;
			}
	...
	private YunSignResult queryContract(String orderId) {
		...
		try {
			return YunSignCaller.callYunSign(domainProperties.getYunSign() + YunSignConfig.URI, "queryContract", strs,
					val);
		} catch (Exception e) {
			return YunSignResult.err(e.getMessage());
		}
	...
	public Map<String, Object> signContract(WorkOrderDTO wo, WorkOrderReqDTO req,boolean isExperience, int homeCostPrice, int homeOpenAccount, int busineseCostPrice, int busineseOpenAccount, String orderId) {
		Map<String, Object> ru = new HashMap();
		if (StringUtil.isNotEmpty(req.getIdCard())) {
			YunSignResult checkResult = checkidentityNumber(req.getIdCard(), req.getName());
			if (!checkResult.isSuccess()) {
				ru.put("success", false);
				ru.put("msg", checkResult.getDesc());
				return ru;
		...
		ru = registerCreate(req.getName(), wo.getDeviceModel(), wo.getSn(), wo.getProvince(), wo.getCity(),
				wo.getRegion(), wo.getAddress(), isExperience, "", wo.getCostType(),
				homeCostPrice, homeOpenAccount, busineseCostPrice, busineseOpenAccount, wo.getOpenAccountFee().intValue(), req.getPhone(),
				req.getIdCard(), req.getYear(), orderId, req.getMail());
		return ru;
	}
}
